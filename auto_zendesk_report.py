...
def report_filename(extents):
    if extents == 'md':
        filename = os.path.join(configure.OUTPUT_PATH, 'markdown_report_')
    else:
        filename = os.path.join(configure.OUTPUT_PATH, 'html_report_')
...
def get_header_css():
    ...
    <link href="../src/isv.css" rel="stylesheet" type="text/css" />
    </head>
    """
    return res
...
def build_html_report():
    ...
    with open(report_filename('html'), 'w', newline='\n') as file:
        ...
        file.writelines("<body class='typora-export'><div id='write' class='is-node'>")
        file.writelines("<p>Hello Scott,</p>")
        file.writelines("<p>Here is the Report of the ISV SDK Support status.</p>")
        file.writelines("<h3><a name='header-n4' class='md-header-anchor '></a>0. Priority Escalations:</h3>")
        file.writelines("<p><em>This issue is not in Forum as a post, but handled through email directly.</em></p>")

        file.writelines("<h3><a name='header-n7' class='md-header-anchor '></a>1. Current New Posts:</h3>")
        file.writelines("<p><em>Current New (no initial response) Posts</em></p>")
        file.writelines("<p><em>Note: Team is performing internal discussion on this issue and decide how to "
                        "response to partner.</em></p>")

        [chart2, cnt2, chart3, cnt3] = build_chart3(data)
        file.writelines("<h3><a name='header-n12' class='md-header-anchor '></a>2. Post handled: " + str(cnt2) + "</h3>")
        file.writelines("<p><em>Posts looked at / responded to in last 24 hours</em></p>")
        file.writelines("<p>" + chart2 + "</p>")

        file.writelines("<h3><a name='header-n18' class='md-header-anchor '></a>3. Posts No Response: "
                        + str(cnt3) + "</h3>")
        file.writelines("<p><em>Current Open Posts that have no response within 24 hours</em></p>")
        file.writelines("<p>" + chart3 + "</p>")
        file.writelines("<p>These Posts are currently Open or being worked on, but without responding within in "
                        "24 hours. We will look into these and take action accordingly.<br/>")
        file.writelines("<em>We are looking into and trying to clean up the post that without no response/update \
        for more than 15 days. (some of them are very old and has no update for months)</em></p>")

        [chart4, cnt4, chart5, cnt5] = build_chart4(data)
        file.writelines("<h3><a name='header-n24' class='md-header-anchor '></a>4. Internal & External Pending Posts: "
                        + str(cnt4) + "</h3>")
        file.writelines("<p><em>Posts waiting on internal external response "
                        "(waiting for FW, RDL, SDK, etc...)</em></p>")
        file.writelines("<p>" + chart4 + "</p>")

        file.writelines("<h3><a name='header-n31' class='md-header-anchor '></a>5. Partner Pending Posts: "
                   + str(cnt5) + "</h3>")
        file.writelines("<p><em>Posts waiting on Partner Response</em></p>")
        file.write(enter + chart5 + enter + enter)
        file.writelines("<p>These posts are waiting for Partner's feedback on query about more info. "
                   "Team will monitor and continue work once partner provided feedback.</p>")
        file.writelines("<h3><a name='header-n37' class='md-header-anchor '></a>6. Overview of Posts from Forum</h3>")
        file.writelines("<p><em>OXPd Pro and Samsung Printing SDK are not included. "
                   "Status of Some of Posts are TBD which are still being reviewed.</em></p>")
        file.writelines("<p><img src='pie.png' alt='img'/></p>")
        file.writelines("<p><img src='bar.png' alt='img'/></p>")
        file.writelines("<p><img src='time_bar.png' alt='img'/></p>")
        # file.write("![img](pie_chart.svg)" + enter + enter)

        file.writelines("<p>Notes:</p>")
        file.writelines("<ol start=''>")
        file.writelines('<li>"Closed" are the posts with Closed status or marked as Answered.</li>')
        file.writelines('<li>"Solved" are the posts that we responded partner with a solution/workaround.</li>')
        file.writelines('<li>"Open" are the posts that need further response from support team.</li>')
        file.writelines('<li>"New" are the ones just created and no response made yet.</li>')
        file.writelines('<li>"Ongoing Work"" are the ones that team are working on for a solution.</li>')
        file.writelines('<li>"External pending" refers to post is waiting on feedback from other teams in HP '
                   '(FW, RDL, SDK, etc.)</li>')
        file.writelines('<li>"Internal Pending" refers to post is waiting on result of Support team.( Response '
                   'is pending for approval, Internal discussion, Article/Documentation/Portal updates, etc.)</li>')
        file.writelines('<li>"Partner Pending"" refers to posts that waiting on feedback from partner to continue.</li>')
        file.writelines('<li>"Not a Support Ticket"  refers to posts that submitted by HP guys and mostly for '
                   'discussion on some topics. Not from partners.</li></ol>')
        file.writelines("<p>Please let us know if any comments on this report. Thanks.</p>")
        file.writelines("<p>Best Regards</p>")
        file.writelines("<p>Denon</p>")
        file.writelines("<p>Beyondsoft Corporation</p>")
        file.writelines("</div>")
